  ...
  return (
    <div className="bg-base-100 min-h-screen">
      ...
      <div className="hero bg-slate-900 text-white py-20 px-4">
        ...
        <div className="container mx-auto flex flex-col items-center text-center">
          <div className="max-w-2xl">
            {" "}
            {/* Increased width slightly for centered look */}
            ...
            <div className="join w-full max-w-md mx-auto">
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && handleSearch()} // Search on Enter key (calling the handleSearch function)
              />
              <button
            ...
            <div className="mt-6 mb-6 flex flex-wrap gap-2 justify-center">
              ...
              {["Web Design", "WordPress", "Logo Design", "AI Services"].map(
                (tag) => (
                  <button
                    key={tag}
                    className="badge badge-outline hover:bg-white hover:text-black cursor-pointer"
                  >
                    {tag}
                  </button>
                )
      ...
      <div className="border-b border-base-200">
        <div className="container mx-auto px-4 flex gap-8 overflow-x-auto py-4 no-scrollbar">
          {categories.map((cat) => (
            <span
              key={cat.slug}
              className="whitespace-nowrap text-gray-500 hover:text-primary cursor-pointer font-medium transition-colors"
            >
              {cat.name}
            </span>
          ))}
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="bg-slate-900 primary rounded-3xl p-10 flex flex-col md:flex-row items-center justify-between text-primary-content">
          <div>
            <h2 className="text-3xl font-bold mb-2">GigFlow business.</h2>
            <p className="text-lg opacity-90">
              A solution built for business. Upgrade to a curated experience.
            </p>
          </div>
          <button className="btn btn-secondary mt-6 md:mt-0">
            Explore Business
          </button>
        </div>
      </div>
