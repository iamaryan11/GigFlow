...
import { DollarSign, ShoppingBag, LayoutGrid, ArrowUpRight } from "lucide-react";
import { Link } from "react-router-dom";

const Dashboard = () => {
  const { isLoading, data } = useQuery({
    queryKey: ["sellerStats"],
    queryFn: () => newRequest.get("/users/stats").then((res) => res.data),
  });
    ...
    <div className="min-h-screen bg-slate-50 pt-28 pb-20 px-4">
      <div className="container mx-auto max-w-6xl space-y-10">
        ...
        <header>
          ...
          <p className="text-slate-500 font-medium">Manage your freelance business at a glance.</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between">
            <div>
              ...
              <h2 className="text-4xl font-black text-slate-900">${data?.earnings}</h2>
            </div>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between">
            <div>
              <p className="text-slate-400 text-sm font-bold uppercase">Total Orders</p>
              <h2 className="text-4xl font-black text-slate-900">{data?.sales}</h2>
            </div>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between">
            <div>
              <p className="text-slate-400 text-sm font-bold uppercase">Active Gigs</p>
              <h2 className="text-4xl font-black text-slate-900">{data?.activeGigs}</h2>
            </div>
            <div className="p-4 bg-purple-100 text-purple-600 rounded-2xl"><LayoutGrid size={32}/></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col justify-between items-start space-y-6">
             ...
             <p className="text-slate-400">Create a new gig to reach more buyers and increase your monthly revenue.</p>
          ...
          <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col justify-between items-start space-y-6">
             ...
             <p className="text-slate-500">Track your current deliveries and communicate with your active clients.</p>
